...
type
  ...
  TaMain = class(TForm)
    ...
    procedure MenuDebugModeClick(Sender: TObject);
...
begin
  ...
  begin
    ...
    Debug := getkey('Debug', '_process_log _port_log _sendmes _noport');
...
begin
  ...
  if ExistDebugKey('process_log') then
    if CurDev <> nil then
    begin
      if IsWrite then
        s := Format('process >> %s PP #%d %s %s (%d)',
...
procedure TaMain.MenuDebugModeClick(Sender: TObject);
begin
