...
begin
  while (not Terminated) do
    try
      ...
      strLog := Format('Порт %s открыт', [Serial]);
      ThreadLog(strLog);
        ...
        while (not Terminated) do
          try
            ...
            begin
              ...
              if CurDev <> nil then
                strLog := strLog + Format('CurDev.Op: %d, CurDev.ObjNum: %d',
      ...
      strLog := Format('Порт %s закрыт', [Serial]);
      if Terminated then
        strLog := strLog + ' по сигналу завершения';
...
procedure TComPort.DumpExceptionCallStack(E: Exception);
var
  I: integer;
  Frames: PPointer;
  Report: string;
